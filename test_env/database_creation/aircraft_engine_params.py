...
def calculate():
    ...
    models = pd.read_json(r"C:\Users\PRohr\Desktop\Masterarbeit\Python\test_env\database_creation\rawdata\aircraft-types.json")
    models = models.explode('engineModels').reset_index(drop=True)
    models = models.explode('propertyValues').reset_index(drop=True)
    manufacturers = pd.read_json(r'C:\Users\PRohr\Desktop\Masterarbeit\Python\test_env\database_creation\rawdata\manufacturers.json')
    manufacturers = manufacturers[['id', 'name']]
    engines = pd.read_json(r"C:\Users\PRohr\Desktop\Masterarbeit\Python\test_env\database_creation\rawdata\engine-models.json")
    engines = engines.explode('propertyValues').reset_index(drop=True)
    properties = pd.read_json(r"C:\Users\PRohr\Desktop\Masterarbeit\Python\test_env\database_creation\rawdata\properties.json")
    ...
    icao = pd.read_excel(r'C:\Users\PRohr\Desktop\Masterarbeit\Python\test_env\database_creation\emissions\icao_cruise_emissions.xlsx')
