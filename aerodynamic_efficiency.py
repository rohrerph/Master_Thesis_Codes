...
breguet = aircraft_data[['MTOW/MZFW', 'Range', 'Name', 'YOI', 'PL/OEW']]
...
breguet['K']= breguet['Range']/breguet['MTOW/MZFW log']
...
ax.scatter(breguet['YOI'], breguet['MTOW/MZFW'], marker='o', label='Aircraft')

for i, row in breguet.iterrows():
    plt.annotate(row['Name'], (row['YOI'], row['MTOW/MZFW']),fontsize=6, xytext=(-8, 5), textcoords='offset points')
ax.legend()
